...
#import "CLGreenMagenta3D.h"

#import "UIView+Frame.h"

@implementation CLGreenMagenta3D
{
...
{
    return NSLocalizedStringWithDefaultValue(@"CLGreenMagenta3D_DefaultTitle", nil, [CLImageEditorTheme bundle], @"Green-Magenta", @"");
}
...
- (UIImage*)applyEffect:(UIImage*)image
{
    ...
    cyanImage = [stillImageFilter imageByFilteringImage:image];
    ...
    [cyanImage drawInRect:CGRectMake(0,0,image.size.width,image.size.height) blendMode:kCGBlendModeNormal alpha:1.0];
    [redImage drawInRect:CGRectMake(_sizeSlider.value,0,image.size.width-_sizeSlider.value,image.size.height) blendMode:kCGBlendModeScreen alpha:1.0];
    redImage = UIGraphicsGetImageFromCurrentImageContext();
    
    UIGraphicsEndImageContext();
    
    CGRect cropRect = CGRectMake(_sizeSlider.value, 0, image.size.width-_sizeSlider.value, image.size.height);

...
#import "CLCross2Mirror.h"
#import "UIView+Frame.h"

@interface CLCross2CircleMirror : UIView
@property (nonatomic, strong) UIColor *color;
@end

@interface CLCross2Mirror()
<UIGestureRecognizerDelegate>
@end

@implementation CLCross2Mirror
{
    UIView *_containerView;
    UIView *_container;
    CLCross2CircleMirror *_circleView;
    UILabel* circleLabel;
...
- (UIImage*)applyMirror:(UIImage*)image
{
    ...
    leftImage = [UIImage imageWithCGImage:leftImage.CGImage scale:leftImage.scale orientation:UIImageOrientationDown];
    [leftImage drawInRect:CGRectMake(0,0,newSize.width/2+1,newSize.height/2)];
    
    rightUpImage = [UIImage imageWithCGImage:leftImage.CGImage scale:leftImage.scale orientation:UIImageOrientationDownMirrored];
    [rightUpImage drawInRect:CGRectMake((newSize.width/2),0,newSize.width/2,newSize.height/2)];
    
    leftDownImage = [UIImage imageWithCGImage:leftImage.CGImage scale:leftImage.scale orientation:UIImageOrientationUpMirrored];
    [leftDownImage drawInRect:CGRectMake(0,(newSize.height/2),newSize.width/2+1,newSize.height/2)];
    
    rightDownImage = [UIImage imageWithCGImage:rightUpImage.CGImage scale:rightUpImage.scale orientation:UIImageOrientationUp];
...
- (void)setUserInterface
{
    ...
    _circleView = [[CLCross2CircleMirror alloc] init];
...
@implementation CLCross2CircleMirror

//
//  CLHorizontalTribleMirror.m
//
//  Created by Kevin Siml - Appzer.de on 2015/10/23.
//  Copyright (c) 2015 Appzer.de. All rights reserved.
//

#import "CLHorizontalTribleMirror.h"
#import "UIView+Frame.h"

@interface CLHorizontalTribleCircleMirror : UIView
@property (nonatomic, strong) UIColor *color;
@end

@interface CLHorizontalTribleMirror()
<UIGestureRecognizerDelegate>
@end

@implementation CLHorizontalTribleMirror
{
    UIView *_containerView;
    UIView *_container;
    CLHorizontalTribleCircleMirror *_circleView;
    UILabel* circleLabel;
...
- (UIImage*)applyMirror:(UIImage*)image
{
    ...
    UIImage *middleImage = image;
    
    CGSize newSize = CGSizeMake(image.size.width, image.size.height);
    UIGraphicsBeginImageContext( newSize );
        
    CGRect cropRect = CGRectMake (0, _Y*2*(image.size.height/3), image.size.width, image.size.height/3);
    ...
    [upperImage drawInRect:CGRectMake(0,0,newSize.width,newSize.height/3)];
    
    middleImage = [UIImage imageWithCGImage:upperImage.CGImage scale:upperImage.scale orientation:UIImageOrientationDownMirrored];
    [middleImage drawInRect:CGRectMake(0,(newSize.height/3)-1,newSize.width,newSize.height/3+1)];
    
    lowerImage = [UIImage imageWithCGImage:upperImage.CGImage scale:upperImage.scale orientation:UIImageOrientationUp];
    [lowerImage drawInRect:CGRectMake(0,(newSize.height/1.5)-1,newSize.width,newSize.height/3+1)];
...
- (void)setUserInterface
{
    ...
    _circleView = [[CLHorizontalTribleCircleMirror alloc] init];
...
@implementation CLHorizontalTribleCircleMirror

...
#import "CLToonEffect.h"

#import "UIView+Frame.h"

@implementation CLToonEffect
{
...
{
    return NSLocalizedStringWithDefaultValue(@"CLToonEffect_DefaultTitle", nil, [CLImageEditorTheme bundle], @"Toon", @"");
}
...
- (UIImage*)applyEffect:(UIImage*)image
{
    ...
    [stillImageFilter setQuantizationLevels:_shadowSlider.value];
    [stillImageFilter setThreshold:_radiusSlider.value];
    [stillImageFilter setBlurRadiusInPixels:_highlightSlider.value];
...
{
    _shadowSlider = [self sliderWithValue:4 minimumValue:1 maximumValue:16];
    _shadowSlider.superview.center = CGPointMake(_containerView.width/2, _containerView.height-30);
    
    _radiusSlider = [self sliderWithValue:0.8 minimumValue:0.1 maximumValue:1];
    ...
    _highlightSlider = [self sliderWithValue:2 minimumValue:1 maximumValue:5];
    _highlightSlider.superview.center = CGPointMake([[UIScreen mainScreen] applicationFrame].size.width-20, _shadowSlider.superview.top - 150);
